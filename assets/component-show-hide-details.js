...
class ShowHideDetails extends HTMLElement {
  constructor() {
    super();
  }
  ...
  init() {
    this.attachEvents();
    window.eight.armada.elementRegistry.register({key: 'show-hide-details', assetPath: '/assets/component-show-hide-details.min.js'});
  }

  attachEvents() {
    ['click', 'keypress'].forEach(ev => {
      this.addEventListener(ev, this.handleShowHideDetails.bind(this));
    });

    if (window.Shopify.designMode) {
      document.addEventListener('shopify:section:load', this.editorEventHandler.bind(this));
      document.addEventListener('shopify:block:select', this.editorEventHandler.bind(this));
    }
  }
  ...
  toggleButtons() {
    this.buttons.forEach(button => {
      ...
      const buttonContent = button.querySelectorAll('[data-armada-selector="show-hide-button-content"]');
      buttonContent.forEach(el => {
  ...
  toggleDetails() {
    this.details.forEach(detail => {
      ...
      this.initSliders(detail);
    })
  }

  initSliders(detail) {
    const sliders = detail.querySelectorAll('slider-engine');
    if (!sliders) return;
  ...
  editorEventHandler(e) {
    ...
    const isHidden = this.buttons[0]?.getAttribute('aria-expanded') === 'false';
    if (isHidden) {
  ...
  get buttons() {
    return document.querySelectorAll(`[data-armada-show-hide-id="${this.id}"]`);
  }

  get details() {
    return document.querySelectorAll(`[data-armada-details-id="${this.id}"]`);
  }

  get id() {
    return this.dataset.armadaShowHideId;
  }

  get sectionId() {
    return this.dataset.armadaShowHideSectionId;
  }

  detachEvents() {
    ['click', 'keypress'].forEach(ev => {
      this.removeEventListener(ev, this.handleShowHideDetails.bind(this));
    });
  }
...
window.customElements.define('show-hide-details', ShowHideDetails);
