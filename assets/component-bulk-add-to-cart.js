...
class BulkAddToCart extends HTMLElement {
  constructor() {
    super();
    this.bulkAddButton = this.querySelector('[data-armada-selector="bulk-add-button"]');
    ...
    this.addedButtonHtml = this.templateContent.querySelector('[data-button-added]').outerHTML;
  }
  ...
  init() {
    this.attachEvents();
    window.eight.armada.elementRegistry.register({key: 'bulk-add-to-cart', assetPath: '/assets/component-bulk-add-to-cart.min.js'});
  }

  attachEvents() {
    this.bulkAddButton.addEventListener('click', this.handleButtonClick.bind(this));
  }
  ...
  setLoadingButton() {
    this.innerHTML = this.loadingButtonHtml;
  }
  ...
  setAddedButton() {
    this.innerHTML = this.addedButtonHtml;
  }

  resetButton() {
    this.innerHTML = this.bulkAddButton.outerHTML;
    this.bulkAddButton = this.querySelector('[data-armada-selector="bulk-add-button"]');
    this.bulkAddButton.addEventListener('click', this.handleButtonClick.bind(this));
  }

  get formData() {
    return window.eight.safeJSONParse(this.dataset.armadaFormData);
  }

  detachEvents() {
    this.bulkAddButton.removeEventListener('click', this.bulkAddToCart.bind(this));
  }
...
window.customElements.define('bulk-add-to-cart', BulkAddToCart);
