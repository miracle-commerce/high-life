...
class Navigation extends HTMLElement {
  ...
  init() {
    ...
    window.eight.armada.elementRegistry.register({key: 'navigation', assetPath: '/assets/component-navigation.min.js'});
  }

  attachEvents() {
    window.addEventListener('resize', () => this.mobileNavHeight());
  ...
  mobileNavHeight() {
    this.mobileSearchBar = document.querySelector('[data-armada-selector="mobile-search-bar"]');
    this.basicHeader = document.querySelector('header-basic');

    if (window.innerWidth > 1023) {
      this.navigation.style.height = 'auto';
    } else {
      if (!this.basicHeader) return;
      const headerHeight = this.basicHeader.offsetHeight;

      if (!this.mobileSearchBar) {
        this.navigation.style.height = `calc(100vh - ${headerHeight}px)`;
      } else {
        const searchBarHeight = this.mobileSearchBar.offsetHeight;
        const combinedHeight = headerHeight + searchBarHeight;
        this.navigation.style.height = `calc(100vh - ${combinedHeight}px)`;
      }
    }
  }
  ...
  get sectionId() {
    return this.dataset.armadaSectionId;
  }
}

window.customElements.define('armada-navigation', Navigation);
